...
class DetectorDetectApi extends api.DetectorApi {
  ...
  _extractProvider(response) {
    if (get.getOwnPropertyDeep(response, "code.value") !== "0") {
   ...
   */
  async byPhone(phone) {
    const response = await this.http.post(url.url`mobile/detect.action`, {
      phone
    });
  ...
  async byCardNumber(cardNumber) {
    const response = await this.http.post(url.url`card/detect.action`, {
      cardNumber
    });
    return this._extractProvider(response);
  }
