...
class WalletBillsApi extends api.WalletApi {
  ...
  async createP2PKeyPair(name, server) {
    ...
    const response = await this.http.post(
      url.url`widgets-api/api/p2p/protected/keys/create`,
      {
        keysPairName: name,
  ...
  async get(parameters = {}) {
    const { bills } = await this.http.get(
      url.url`checkout-api/api/bill/search`(parameters)
    );
    return bills;
  }
  /**
  ...
  async pay(bill, currency) {
    ...
    return await this.http.post(url.url`checkout-api/invoice/pay/wallet`, {
   ...
   */
  async reject(id) {
    await this.http.post(url.url`checkout-api/api/bill/reject`, { id });
  }
}
