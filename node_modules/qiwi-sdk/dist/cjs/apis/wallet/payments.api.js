...
class WalletPaymentsApi extends api.WalletApi {
  ...
  async getCommission(
    ...
    const response = await this.http.post(
      url.url`sinap/providers/${provider}/onlineCommission`,
      {
        account,
        paymentMethod: {
  ...
  static createFormUrl(provider, options = {}) {
    ...
    return url.url`https://qiwi.com/payment/form/${provider}`(data);
  }
  /**
   * Данный метод создаёт ссылку на предзаполненную форму оплаты
   *
  ...
  async pay({
    ...
    return await this.http.post(url.url`sinap/api/v2/terms/${provider}/payments`, {
   ...
   */
  async getRates() {
    const { result: rates } = await this.http.get(url.url`sinap/crossRates`);
