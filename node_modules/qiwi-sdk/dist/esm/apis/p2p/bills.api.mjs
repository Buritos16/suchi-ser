import { URL as url } from "../shared/url-globals.mjs";
import { generateUUID } from "../shared/uuid.mjs";
import { url as url$1 } from "../shared/url.mjs";
import { formatOffsetDate, formatOffsetAltLifetimeDate } from "../shared/time.mjs";
import { compareQiwiHmac } from "../shared/hmac.mjs";
...
class P2pBillsApi extends P2pApi {
  ...
  static patchPayUrl(payUrl, options = {}) {
    const url$1 = new url(payUrl);
    if (options.paySource) url$1.searchParams.set("paySource", options.paySource);
    if (options.successUrl) url$1.searchParams.set("successUrl", options.successUrl);
  ...
  async create(data) {
    ...
    const result = await this.http.put(url$1`${billId}`, patchedBill);
  ...
  async getStatus(billId) {
    return await this.http.get(url$1`${billId}`);
  }
  /**
   ...
   */
  async reject(billId) {
    return await this.http.post(url$1`${billId}/reject`);
  }
  /**
   *
   *
  ...
  createFormUrl(parameters) {
    ...
    return url$1`https://oplata.qiwi.com/create`(options);
  }
  /**
   *
  ...
  async refund(billId, options) {
    ...
    return await this.http.put(url$1`${billId}/refunds/${refundId}`);
  }
  /**
   *
   * @deprecated API заархивировано
  ...
  async getRefundStatus(billId, refundId) {
    return await this.http.get(url$1`${billId}/refunds/${refundId}`);
  }
}
export { P2pBillsApi };
