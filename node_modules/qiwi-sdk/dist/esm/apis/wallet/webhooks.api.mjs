...
import { getOwnPropertyDeep } from "../shared/get.mjs";
import { WalletApi } from "./api.mjs";
import { base64ToUint8Array } from "../shared/platform/decode/decode.mjs";
/**
 * # Уведомления (вебхуки)
 *
...
class WalletWebhooksApi extends WalletApi {
  ...
  remove(hookId = this._getDefaultHookId()) {
    ...
    return this.http.delete(url`payment-notifier/v1/hooks/${hookId}`);
  }
  /**
  ...
  async getActiveWebhook() {
    const hookResponse = await this.http.get(url`payment-notifier/v1/hooks/active`);
    this.activeId = hookResponse.hookId;
    return hookResponse;
  }
  /**
   ...
   */
  testActiveWebhook() {
    return this.http.get(url`payment-notifier/v1/hooks/test`);
  }
  /**
  ...
  async checkSign(transaction) {
    ...
    const signPayload = payment.signFields.split(",").map((fieldName) => {
      var _a;
      return String(
        (_a = getOwnPropertyDeep(payment, fieldName)) !== null && _a !== void 0
          ? _a
          : "null"
      );
    });
